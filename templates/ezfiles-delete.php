<h1><?=esc_html( $action_display )?> Handle</h1>
...
<form action="<?php echo esc_html( admin_url( 'admin-post.php' ) ); ?>" method="post">
	...
	?>
	<?php if ( $can_delete ): ?>
		Are you sure you want to delete handle <b><?=esc_html( $item->emulator_id )?></b>?
	<?php else: ?>
		Handle <b><?=esc_html( $item->emulator_id )?></b> has associated downloads and cannot be deleted.
	<?php endif; ?>
